...
*Setup instructions for a gitian build of VoltPotCoin Core using a VM or physical system.*
...
to the VoltPotCoin GitHub release page.
...
- [Building VoltPotCoin Core](#building-voltpotcoin-core)
...
```bash
gpg --output ${VERSION}-linux/${NAME}/voltpotcoin-linux-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-linux/$NAME/voltpotcoin-linux-${VERSION%\.*}-build.assert
gpg --output ${VERSION}-osx-unsigned/$NAME/voltpotcoin-osx-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-osx-unsigned/$NAME/voltpotcoin-osx-${VERSION%\.*}-build.assert
gpg --output ${VERSION}-win-unsigned/$NAME/voltpotcoin-win-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-win-unsigned/$NAME/voltpotcoin-win-${VERSION%\.*}-build.assert
```

Uploading Signatures
--------------------
Make a Pull Request (both the `.assert` and `.assert.sig` files) to the
[gitian.sigs](https://github.com/voltpotcoin-project/gitian.sigs/) repository:
...
You can also mail the files to Fuzzbawls (fuzzbawls@voltpotcoin.org) and he will commit them.

```bash
gpg --detach-sign ${VERSION}-linux/${NAME}/voltpotcoin-linux-*-build.assert
gpg --detach-sign ${VERSION}-win-unsigned/${NAME}/voltpotcoin-win-*-build.assert
gpg --detach-sign ${VERSION}-osx-unsigned/${NAME}/voltpotcoin-osx-*-build.assert
```
