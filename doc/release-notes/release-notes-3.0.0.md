VoltPotCoin Core version 3.0.0 is now available from:
...
### Fees
zVpc transactions require more computation and disk space than typical VoltPotCoin transactions, and as such require a higher transaction fee in order to prevent network spam. Fees are only charged when minting zVpc, each minted denomination is charged a flat rate of 0.01 Vpc. zVpc spends are not charged a transaction fee unless the change is minted into zVpc, see the *Minting Change* section for details on fees for zVpc spends with minted change.
