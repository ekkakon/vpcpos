...
class ZerocoinSpendTest(VoltpotcoinTestFramework):
    ...
    def run_test(self):
        ...
        def check_balances(denom, zvpc_bal, vpc_bal):
            zvpc_bal -= denom
            assert_equal(self.nodes[2].getzerocoinbalance()['Total'], zvpc_bal)
            vpc_bal += denom
            wi = self.nodes[2].getwalletinfo()
            assert_equal(wi['balance'] + wi['immature_balance'], vpc_bal)
            return zvpc_bal, vpc_bal
        ...
        zvpc_balance = self.nodes[2].getzerocoinbalance()['Total']
        assert_equal(balance, DecimalAmt(13833.92))
        assert_equal(zvpc_balance, 6666)
        ...
        zvpc_balance, balance = check_balances(denom_0, zvpc_balance, balance)
        ...
        block_time = stake_4_blocks(block_time)
        self.check_tx_in_chain(0, txid)
        zvpc_balance, balance = check_balances(denom_2, zvpc_balance, balance)
        ...
        block_time = stake_4_blocks(block_time)
        self.check_tx_in_chain(0, txid)
        zvpc_balance, balance = check_balances(denom_3, zvpc_balance, balance)
        ...
        assert_raises_rpc_error(-26, "bad-txns-invalid-zvpc",
        ...
        _, _ = check_balances(denom_1, zvpc_balance, balance)
        self.log.info("--> VALID PUBLIC COIN SPEND (v3) PASSED")
