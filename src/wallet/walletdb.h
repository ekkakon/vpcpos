...
public:
    ...
    bool WriteZVPCSeed(const uint256& hashSeed, const std::vector<unsigned char>& seed);
    bool ReadZVPCSeed(const uint256& hashSeed, std::vector<unsigned char>& seed);
    bool ReadZVPCSeed_deprecated(uint256& seed);
    bool EraseZVPCSeed();
    bool EraseZVPCSeed_deprecated();

    bool WriteZVPCCount(const uint32_t& nCount);
    bool ReadZVPCCount(uint32_t& nCount);
