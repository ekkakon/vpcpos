...
extern bool fGlobalUnlockSpendCache; // Bool used for letting the precomputing thread know that zvpcspends need to use the cs_spendcache
...
enum AvailableCoinsType {
    ...
    ONLY_DENOMINATED = 2,
    ONLY_NOT10000IFMN = 3,
    ONLY_NONDENOMINATED_NOT10000IFMN = 4, // ONLY_NONDENOMINATED and not 20000 VPC at the same time
    ONLY_10000 = 5,                        // find masternode outputs including locked ones (use with caution)
    STAKABLE_COINS = 6                          // UTXO's that are valid for staking
};

// Possible states for zVPC send
enum ZerocoinSpendStatus {
    ZVPC_SPEND_OKAY = 0,                            // No error
    ZVPC_SPEND_ERROR = 1,                           // Unspecified class of errors, more details are (hopefully) in the returning text
    ZVPC_WALLET_LOCKED = 2,                         // Wallet was locked
    ZVPC_COMMIT_FAILED = 3,                         // Commit failed, reset status
    ZVPC_ERASE_SPENDS_FAILED = 4,                   // Erasing spends during reset failed
    ZVPC_ERASE_NEW_MINTS_FAILED = 5,                // Erasing new mints during reset failed
    ...
    ZVPC_BAD_SERIALIZATION = 13,                    // Transaction verification failed
    ZVPC_SPENT_USED_ZVPC = 14,                      // Coin has already been spend
    ZVPC_TX_TOO_LARGE = 15,                          // The transaction is larger than the max tx size
    ZVPC_SPEND_V1_SEC_LEVEL                         // Spend is V1 and security level is not set to 100
};
...
public:
    ...
    std::string GetUniqueWalletBackupName(bool fzvpcAuto) const;
    ...
    */
    ...
    bool fBackupMints;
    std::unique_ptr<CzVPCTracker> zvpcTracker;
    ...
    CzVPCWallet* getZWallet() { return zwalletMain; }
