    ...
    if(IsCrypted()) {
        if (!IsLocked()) { //if we have password
            ...
            if (EncryptSecret(vMasterKey, kmSeed, hashSeed, vchSeedSecret)) {
                ...
                if (db.WriteZVPCSeed(hashSeed, vchSeedSecret)) {
                    return true;
                }
            }
            strErr = "encrypt seed";
        ...
        if (db.WriteZVPCSeed(hashSeed, ToByteVector(seed))) {
            return true;
        }
        strErr = "save zvpcseed to wallet";
    }
    //the use case for this is no password set seed, mint dzVPC,
...
bool CWallet::GetDeterministicSeed(const uint256& hashSeed, uint256& seedOut)
{
    ...
    if (IsCrypted()) {
        if(!IsLocked()) { //if we have password
            ...
            //read encrypted seed
            if (db.ReadZVPCSeed(hashSeed, vchCryptedSeed)) {
        ...
        // wallet not crypted
        if (db.ReadZVPCSeed(hashSeed, vchSeed)) {
        ...
        CBitcoinAddress* changeAddress)
{
    ...
    const int nMaxSpends = Params().GetConsensus().ZC_MaxPublicSpendsPerTx; // Maximum possible spends for one zVPC public spend transaction
...
{
    zwalletMain = zwallet;
    zvpcTracker = std::unique_ptr<CzVPCTracker>(new CzVPCTracker(this));
}
