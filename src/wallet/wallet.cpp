                                    ...
                                    CZerocoinSpendReceipt& receipt, libzerocoin::SpendType spendType, CBlockIndex* pindexCheckpoint)
{
    ...
    for (auto &it : mapMintsSelected) {
        ...
        CTxIn in;
        if(!ZVPCModule::createInput(in, mint, hashTxOut, spendVersion)) {
...
void CWallet::setZWallet(CzVPCWallet* zwallet)
{
    zwalletMain = zwallet;
    zvpcTracker = std::unique_ptr<CzVPCTracker>(new CzVPCTracker(strWalletFile));
}

CzVPCWallet* CWallet::getZWallet()
{
    return zwalletMain;
}
