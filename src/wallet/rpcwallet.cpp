...
UniValue sendmany(const UniValue& params, bool fHelp)
{
    ...
    for (const string& name_ : keys) {
        ...
        if (!address.IsValid())
            throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, string("Invalid VoltPotCoin address: ")+name_);
...
UniValue listmintedzerocoins(const UniValue& params, bool fHelp)
{
    ...
    CWalletDB walletdb(pwalletMain->strWalletFile);
    set<CMintMeta> setMints = pwalletMain->zvpcTracker->ListMints(true, fMatureOnly, true);
...
UniValue listzerocoinamounts(const UniValue& params, bool fHelp)
{
    ...
    CWalletDB walletdb(pwalletMain->strWalletFile);
    set<CMintMeta> setMints = pwalletMain->zvpcTracker->ListMints(true, true, true);
...
extern UniValue DoZvpcSpend(const CAmount nAmount, bool fMintChange, bool fMinimizeChange, vector<CZerocoinMint>& vMintsSelected, std::string address_str, bool ispublicspend)
{
...
UniValue resetmintzerocoin(const UniValue& params, bool fHelp)
{
    ...
    set<CMintMeta> setMints = zvpcTracker->ListMints(false, false, true);
...
UniValue resetspentzerocoin(const UniValue& params, bool fHelp)
{
    ...
    CWalletDB walletdb(pwalletMain->strWalletFile);
    CzVPCTracker* zvpcTracker = pwalletMain->zvpcTracker.get();
    set<CMintMeta> setMints = zvpcTracker->ListMints(false, false, false);
...
UniValue exportzerocoins(const UniValue& params, bool fHelp)
{
    ...
    CzVPCTracker* zvpcTracker = pwalletMain->zvpcTracker.get();
    set<CMintMeta> setMints = zvpcTracker->ListMints(!fIncludeSpent, false, false);
