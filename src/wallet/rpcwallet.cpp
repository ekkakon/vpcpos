...
UniValue setstakesplitthreshold(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw std::runtime_error(
            ...
            "coinstake transaction will have 4 outputs (of 500 VPC each)."
            + HelpRequiringPassphrase() + "\n"

            "\nArguments:\n"
            "1. value                   (numeric, required) Threshold value (in VPC).\n"
...
UniValue spendzerocoin(const UniValue& params, bool fHelp)
{
    ...
    return DoZvpcSpend(nAmount, vMintsSelected, address_str);
}
...
extern UniValue DoZvpcSpend(const CAmount nAmount, std::vector<CZerocoinMint>& vMintsSelected, std::string address_str)
{
...
UniValue spendrawzerocoin(const UniValue& params, bool fHelp)
{
    ...
    return DoZvpcSpend(mint.GetDenominationAsAmount(), vMintsSelected, address_str);
}
