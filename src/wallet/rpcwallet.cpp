...
UniValue sendfrom(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 3 || params.size() > 6)
        throw std::runtime_error(
            "sendfrom \"fromaccount\" \"tovoltpotcoinaddress\" amount ( minconf \"comment\" \"comment-to\" )\n"
            "\nSent an amount from an account to a voltpotcoin address.\n"
...
UniValue spendzerocoin(const UniValue& params, bool fHelp)
{
    ...
    bool fMintChange = params[1].get_bool();        // Mint change to zVPC
    if (fMintChange && Params().NetworkID() != CBaseChainParams::REGTEST)
        throw JSONRPCError(RPC_WALLET_ERROR, "zVPC minting is DISABLED, cannot mint change");
...
UniValue spendzerocoinmints(const UniValue& params, bool fHelp)
{
    ...
    return DoZvpcSpend(nAmount, false, true, vMintsSelected, address_str);
}
...
UniValue spendrawzerocoin(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 4 || params.size() > 5)
        throw std::runtime_error(
            ...
            "5. \"address\"          (string, optional) VoltPotCoin address to spend to. If not specified, spend to change add.\n"
...
UniValue clearspendcache(const UniValue& params, bool fHelp)
{
    if(fHelp || params.size() != 0)
        throw std::runtime_error(
            "clearspendcache\n"
            "\nClear the pre-computed zVPC spend cache, and database.\n" +
            HelpRequiringPassphrase() + "\n"
...

    {
        int nTries = 0;
        while (nTries < 100) {
            TRY_LOCK(zvpcTracker->cs_spendcache, fLocked);
            if (fLocked) {
                if (zvpcTracker->ClearSpendCache()) {
