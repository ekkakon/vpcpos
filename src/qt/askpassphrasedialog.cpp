...
void AskPassphraseDialog::accept()
{
    ...
    case Mode::Encrypt: {
        ...
        bool ret = openStandardDialog(
                ...
                " <b>" + tr("LOSE ALL OF YOUR VPC") + "</b>!<br><br>" + tr("Are you sure you wish to encrypt your wallet?"),
                tr("ENCRYPT"), tr("CANCEL")
        );
        if (ret) {
            if (newpass1 == newpass2) {
                newpassCache = newpass1;
                VoltPotCoinGUI* window = static_cast<VoltPotCoinGUI*>(parentWidget());
...
{
    VoltPotCoinGUI* gui = static_cast<VoltPotCoinGUI*>(parentWidget());
...
{
    hide();
    static_cast<VoltPotCoinGUI*>(parentWidget())->showHide(true);
    openStandardDialog(
            tr("Wallet encrypted"),
            "<qt>" +
