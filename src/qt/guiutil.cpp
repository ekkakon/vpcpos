...
QString formatBalance(CAmount amount, int nDisplayUnit, bool isZvpc){
    return (amount == 0) ? ("0.00 " + BitcoinUnits::name(nDisplayUnit, isZvpc)) : BitcoinUnits::floorHtmlWithUnit(nDisplayUnit, amount, false, BitcoinUnits::separatorAlways, true, isZvpc);
}
...
void setupAddressWidget(QValidatedLineEdit* widget, QWidget* parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter VoltPotCoin address (e.g. %1)").arg("D7VFR83SQbiezrW72hjcWJtcfip5krte2Z"));
...
{
    // return if URI is not valid or is no VoltPotCoin: URI
...
{
    // Convert voltpotcoin:// to voltpotcoin:
    //
    //    Cannot handle this later, because voltpotcoin:// will cause Qt to see the part after // as host,
...
bool openConfigfile()
{
    ...
    /* Open voltpotcoin.conf with the associated application */
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    if (!fAutoStart)
        ...
        if (!optionFile.good())
            return false;
        // Write a voltpotcoin.desktop file to the autostart directory:
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    // loop through the list of startup items and try to find the voltpotcoin app
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if (fAutoStart && !foundItem) {
        // add voltpotcoin app to startup item list
