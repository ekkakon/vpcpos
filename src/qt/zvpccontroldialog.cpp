...
#include "ui_zvpccontroldialog.h"
...
std::set<CMintMeta> ZVpcControlDialog::setMints;

bool CZVpcControlWidgetItem::operator<(const QTreeWidgetItem &other) const {
    int column = treeWidget()->sortColumn();
    if (column == ZVpcControlDialog::COLUMN_DENOMINATION || column == ZVpcControlDialog::COLUMN_VERSION || column == ZVpcControlDialog::COLUMN_CONFIRMATIONS)
        return data(column, Qt::UserRole).toLongLong() < other.data(column, Qt::UserRole).toLongLong();
    return QTreeWidgetItem::operator<(other);
}


ZVpcControlDialog::ZVpcControlDialog(QWidget *parent) :
    QDialog(parent, Qt::WindowSystemMenuHint | Qt::WindowTitleHint | Qt::WindowCloseButtonHint),
    ui(new Ui::ZVpcControlDialog),
    model(0)
{
    ui->setupUi(this);
    setMints.clear();
    ...
    // click on checkbox
    connect(ui->treeWidget, &QTreeWidget::itemChanged, this, &ZVpcControlDialog::updateSelection);
    // push select/deselect all button
    connect(ui->pushButtonAll, &QPushButton::clicked, this, &ZVpcControlDialog::ButtonAllClicked);
}

ZVpcControlDialog::~ZVpcControlDialog()
{
    delete ui;
}

void ZVpcControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZVpcControlDialog::updateList()
{
    ...
    for (auto denom : libzerocoin::zerocoinDenomList) {
        CZVpcControlWidgetItem* itemDenom(new CZVpcControlWidgetItem);
    ...
    for (const CMintMeta& mint : setMints) {
        ...
        CZVpcControlWidgetItem *itemMint = new CZVpcControlWidgetItem(ui->treeWidget->topLevelItem(mapDenomPosition.at(denom)));
...
void ZVpcControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZVpcControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CMintMeta& mint : setMints) {
    ...
    ui->labelZVpc_int->setText(QString::number(nAmount));
...
void ZVpcControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
