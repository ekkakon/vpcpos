...
#include "qt/voltpotcoin/splash.h"
#include "qt/voltpotcoin/welcomecontentwidget.h"
...
{
    return QCoreApplication::translate("voltpotcoin-core", psz).toStdString();
}
...
static void initTranslations(QTranslator& qtTranslatorBase, QTranslator& qtTranslator, QTranslator& translatorBase, QTranslator& translator, bool forceLangFromSettings = false)
{
    ...
    // Load e.g. bitcoin_de.qm (shortcut "de" needs to be defined in voltpotcoin.qrc)
    if (translatorBase.load(lang, ":/translations/"))
        QApplication::installTranslator(&translatorBase);

    // Load e.g. bitcoin_de_DE.qm (shortcut "de_DE" needs to be defined in voltpotcoin.qrc)
...
/** Class encapsulating VoltPotCoin Core startup and shutdown.
...
/** Main VoltPotCoin application object */
...
public:
    ...
    /// Get window identifier of QMainWindow (VoltPotCoinGUI)
...
void BitcoinApplication::initializeResult(int retval)
{
    ...
    if (retval) {
        ...
        if (pwalletMain) {
            ...
            window->addWallet(VoltPotCoinGUI::DEFAULT_WALLET, walletModel);
            window->setCurrentWallet(VoltPotCoinGUI::DEFAULT_WALLET);
        ...
        // VoltPotCoin: URIs or payment requests:
        //connect(paymentServer, &PaymentServer::receivedPaymentRequest, window, &VoltPotCoinGUI::handlePaymentRequest);
        connect(window, &VoltPotCoinGUI::receivedURI, paymentServer, &PaymentServer::handleURIOrFile);
...
{
    QMessageBox::critical(0, "Runaway exception", VoltPotCoinGUI::tr("A fatal error occurred. VoltPotCoin can no longer continue safely and will quit.") + QString("\n\n") + message);
    ::exit(1);
}
...
/// 2. Basic Qt initialization (not dependent on parameters or configuration)
    Q_INIT_RESOURCE(voltpotcoin_locale);
    Q_INIT_RESOURCE(voltpotcoin);
    ...
    /// 6. Determine availability of data directory and parse voltpotcoin.conf
    /// - Do not call GetDataDir(true) before this step finishes
    if (!boost::filesystem::is_directory(GetDataDir(false))) {
        QMessageBox::critical(0, QObject::tr("VoltPotCoin Core"),
    ...
    if (!SelectParamsFromCommandLine()) {
        QMessageBox::critical(0, QObject::tr("VoltPotCoin Core"), QObject::tr("Error: Invalid combination of -regtest and -testnet."));
        return 1;
    }
#ifdef ENABLE_WALLET
    ...
    // voltpotcoin: links repeatedly have their payment requests routed to this process:
...
#if defined(Q_OS_WIN)
        WinShutdownMonitor::registerShutdownBlockReason(QObject::tr("VoltPotCoin Core didn't yet exit safely..."), (HWND)app.getMainWinId());
#endif
        app.exec();
        app.requestShutdown();
        app.exec();
