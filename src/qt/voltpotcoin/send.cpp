...
#include "qt/voltpotcoin/send.h"
#include "qt/voltpotcoin/forms/ui_send.h"
#include "qt/voltpotcoin/addnewcontactdialog.h"
#include "qt/voltpotcoin/qtutils.h"
#include "qt/voltpotcoin/sendchangeaddressdialog.h"
#include "qt/voltpotcoin/optionbutton.h"
#include "qt/voltpotcoin/sendconfirmdialog.h"
#include "qt/voltpotcoin/myaddressrow.h"
...
SendWidget::SendWidget(VoltPotCoinGUI* parent) :
    PWidget(parent),
    ui(new Ui::send),
    ...
    btnContacts(new QPushButton())
{
    ...
    /* Button Group */
    ui->pushLeft->setText("VPC");
    setCssProperty(ui->pushLeft, "btn-check-left");
    ui->pushLeft->setChecked(true);
    ui->pushRight->setText("zVPC");
    setCssProperty(ui->pushRight, "btn-check-right");

    /* Subtitle */
    ui->labelSubtitle1->setText(tr("You can transfer public coins (VPC) or private coins (zVPC)"));
    ...
    /* Address */
    ui->labelSubtitleAddress->setText(tr("VoltPotCoin address or contact label"));
    ...
    ui->labelAmountSend->setText("0.00 VPC");
    ...
    setCssProperty(coinIcon, "coin-icon-vpc");
    ...
    // Connect
    connect(ui->pushLeft, &QPushButton::clicked, [this](){onVPCSelected(true);});
    connect(ui->pushRight,  &QPushButton::clicked, [this](){onVPCSelected(false);});
...
void SendWidget::refreshView()
{
    ...
    ui->pushButtonSave->setText(isChecked ? tr("Send VPC") : tr("Send zVPC"));
...
void SendWidget::refreshAmounts()
{
    ...
    bool isZvpc = ui->pushRight->isChecked();
    nDisplayUnit = walletModel->getOptionsModel()->getDisplayUnit();

    ui->labelAmountSend->setText(GUIUtil::formatBalance(total, nDisplayUnit, isZvpc));
...
void SendWidget::showHideCheckBoxDelegations()
{
    ...
    // coincontrol is not selected, and we are trying to spend VPC (not zVPC)
    const bool isZvpc = ui->pushRight->isChecked();
    const bool isCControl = CoinControlDialog::coinControl->HasSelected();
    const bool hasDel = cachedDelegatedBalance > 0;

    const bool showCheckBox = !isZvpc && !isCControl && hasDel;
    ...
    if (showCheckBox)
        ui->checkBoxDelegations->setToolTip(
                tr("Possibly spend coins delegated for cold-staking (currently available: %1").arg(
                        GUIUtil::formatBalance(cachedDelegatedBalance, nDisplayUnit, isZvpc))
        );
}
...
bool SendWidget::sendZvpc(QList<SendCoinsRecipient> recipients)
{
    if (!walletModel || !walletModel->getOptionsModel())
        return false;
    ...
    if (walletModel->sendZvpc(
        ...
        QString body;
        if (receipt.GetStatus() == ZVPC_SPEND_V1_SEC_LEVEL) {
            body = tr("Version 1 zVPC require a security level of 100 to successfully spend.");
        } else {
            int nNeededSpends = receipt.GetNeededSpends(); // Number of spends we would need for this transaction
            const int nMaxSpends = Params().GetConsensus().ZC_MaxSpendsPerTx; // Maximum possible spends for one zVPC transaction
        ...
        Q_EMIT message("zVPC transaction failed", body, CClientUIInterface::MSG_ERROR);
        return false;
    }
}
...
void SendWidget::onCoinControlClicked()
{
    if (isVPC) {
        if (walletModel->getBalance() > 0) {
            ...
            inform(tr("You don't have any VPC to select."));
        }
    } else {
...
void SendWidget::onVPCSelected(bool _isVPC)
{
    isVPC = _isVPC;
    setCssProperty(coinIcon, _isVPC ? "coin-icon-vpc" : "coin-icon-zvpc");
    refreshView();
...
void SendWidget::onContactMultiClicked()
{
    if (focusedEntry) {
        ...
        CBitcoinAddress vpcAdd = CBitcoinAddress(address.toStdString());
        if (walletModel->isMine(vpcAdd)) {
        ...
        if (dialog->res) {
            ...
            if (walletModel->updateAddressBookLabels(vpcAdd.Get(), dialog->getLabel().toStdString(),
                    AddressBook::AddressBookPurpose::SEND)) {
