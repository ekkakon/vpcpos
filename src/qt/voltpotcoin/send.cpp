...
void SendWidget::refreshAmounts() {
    ...
    if (CoinControlDialog::coinControl->HasSelected()){
        ...
        // Wallet's balance
        totalAmount = (isZvpc ? walletModel->getZerocoinBalance() : walletModel->getBalance()) - total;
...
bool SendWidget::sendZvpc(QList<SendCoinsRecipient> recipients){
    ...
    if(sporkManager.IsSporkActive(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        emit message(tr("Spend Zerocoin"), tr("zVPC is currently undergoing maintenance."), CClientUIInterface::MSG_ERROR);
        return false;
    }
    ...
    if (walletModel->sendZvpc(
        ...
        emit message("zVPC transaction failed", body, CClientUIInterface::MSG_ERROR);
        return false;
    }
}
