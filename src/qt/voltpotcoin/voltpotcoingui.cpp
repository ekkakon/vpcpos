// Copyright (c) 2019-2020 The VoltPotCoin developers
// Distributed under the MIT software license, see the accompanying
// file COPYING or http://www.opensource.org/licenses/mit-license.php.

#include "qt/voltpotcoin/voltpotcoingui.h"
...
const QString VoltPotCoinGUI::DEFAULT_WALLET = "~Default";

VoltPotCoinGUI::VoltPotCoinGUI(const NetworkStyle* networkStyle, QWidget* parent) :
        QMainWindow(parent),
        clientModel(0){
...
void VoltPotCoinGUI::createActions(const NetworkStyle* networkStyle){
 ...
 */
void VoltPotCoinGUI::connectActions() {
    ...
    connect(topBar, &TopBar::showHide, this, &VoltPotCoinGUI::showHide);
    connect(topBar, &TopBar::themeChanged, this, &VoltPotCoinGUI::changeTheme);
    connect(topBar, &TopBar::onShowHideColdStakingChanged, navMenu, &NavMenuWidget::onShowHideColdStakingChanged);
    connect(settingsWidget, &SettingsWidget::showHide, this, &VoltPotCoinGUI::showHide);
    ...
    connect(coldStakingWidget, &ColdStakingWidget::execDialog, this, &VoltPotCoinGUI::execDialog);
    connect(settingsWidget, &SettingsWidget::execDialog, this, &VoltPotCoinGUI::execDialog);
}


void VoltPotCoinGUI::createTrayIcon(const NetworkStyle* networkStyle) {
#ifndef Q_OS_MAC
    trayIcon = new QSystemTrayIcon(this);
    QString toolTip = tr("VoltPotCoin Core client") + " " + networkStyle->getTitleAddText();
...
void VoltPotCoinGUI::setClientModel(ClientModel* clientModel) {
...
#ifndef Q_OS_MAC
void VoltPotCoinGUI::trayIconActivated(QSystemTrayIcon::ActivationReason reason)
{
...
void VoltPotCoinGUI::closeEvent(QCloseEvent* event)
{
#ifndef Q_OS_MAC // Ignored on Mac
...
void VoltPotCoinGUI::messageInfo(const QString& text){
...
void VoltPotCoinGUI::message(const QString& title, const QString& message, unsigned int style, bool* ret) {
    QString strTitle =  tr("VoltPotCoin Core"); // default title
...
bool VoltPotCoinGUI::openStandardDialog(QString title, QString body, QString okBtn, QString cancelBtn){
    ...
    if (isVisible()) {
        ...
        dialog->setWindowTitle(tr("VoltPotCoin Core"));
        dialog->adjustSize();
        dialog->raise();
        dialog->exec();
    }
...
void VoltPotCoinGUI::detectShutdown() {
    if (ShutdownRequested()) {
        if (rpcConsole)
            rpcConsole->hide();
        qApp->quit();
...
void VoltPotCoinGUI::goToDashboard(){
...
void VoltPotCoinGUI::goToSend(){
    showTop(sendWidget);
}

void VoltPotCoinGUI::goToAddresses(){
...
void VoltPotCoinGUI::goToColdStaking(){
    showTop(coldStakingWidget);
}

void VoltPotCoinGUI::goToSettings(){
    showTop(settingsWidget);
}

void VoltPotCoinGUI::goToReceive(){
    showTop(receiveWidget);
}

void VoltPotCoinGUI::showTop(QWidget* view){
...
void VoltPotCoinGUI::changeTheme(bool isLightTheme){
...
void VoltPotCoinGUI::resizeEvent(QResizeEvent* event){
    // Parent..
...
void VoltPotCoinGUI::showHide(bool show){
...
int VoltPotCoinGUI::getNavWidth(){
    return this->navMenu->width();
}

void VoltPotCoinGUI::openFAQ(int section){
    showHide(true);
...
#ifdef ENABLE_WALLET
bool VoltPotCoinGUI::addWallet(const QString& name, WalletModel* walletModel)
{
    ...
    connect(addressesWidget, &AddressesWidget::message,this, &VoltPotCoinGUI::message);
...
bool VoltPotCoinGUI::setCurrentWallet(const QString& name) {
    // Single wallet supported.
    return true;
}

void VoltPotCoinGUI::removeAllWallets() {
    // Single wallet supported.
}

void VoltPotCoinGUI::incomingTransaction(const QString& date, int unit, const CAmount& amount, const QString& type, const QString& address) {
...
static bool ThreadSafeMessageBox(VoltPotCoinGUI* gui, const std::string& message, const std::string& caption, unsigned int style)
{
...
void VoltPotCoinGUI::subscribeToCoreSignals()
{
    // Connect signals to client
    uiInterface.ThreadSafeMessageBox.connect(boost::bind(ThreadSafeMessageBox, this, _1, _2, _3));
}

void VoltPotCoinGUI::unsubscribeFromCoreSignals()
{
