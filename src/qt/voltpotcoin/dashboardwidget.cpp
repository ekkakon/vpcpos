...
#include "qt/voltpotcoin/forms/ui_dashboardwidget.h"
#include "qt/voltpotcoin/sendconfirmdialog.h"
#include "qt/voltpotcoin/txrow.h"
...
DashboardWidget::DashboardWidget(VoltPotCoinGUI* parent) :
    PWidget(parent),
    ui(new Ui::DashboardWidget)
{
    ...
    ui->btnHowTo->setText(tr("How to get VPC or zVPC"));
...
bool hasCharts = false;
    ...
    if (window)
        connect(window, &VoltPotCoinGUI::windowResizeEvent, this, &DashboardWidget::windowResizeEvent);
#endif
...
void DashboardWidget::loadWalletModel()
{
    if (walletModel && walletModel->getOptionsModel()) {
        ...
        connect(ui->pushImgEmpty, &QPushButton::clicked, window, &VoltPotCoinGUI::openFAQ);
        connect(ui->btnHowTo, &QPushButton::clicked, window, &VoltPotCoinGUI::openFAQ);
...
bool DashboardWidget::loadChartData(bool withMonthNames)
{
    ...
    chartData->amountsByCache = getAmountBy(); // pair VPC, zVPC
...
void DashboardWidget::onChartRefreshed()
{
    ...
    // init sets
    set0 = new QBarSet("VPC");
    set1 = new QBarSet("zVPC");
