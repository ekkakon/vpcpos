...
#include "qt/voltpotcoin/topbar.h"
#include "qt/voltpotcoin/forms/ui_topbar.h"
#include "qt/voltpotcoin/lockunlock.h"
#include "qt/voltpotcoin/qtutils.h"
#include "qt/voltpotcoin/receivedialog.h"
...
TopBar::TopBar(VoltPotCoinGUI* _mainWindow, QWidget *parent) :
    ...
    ui(new Ui::TopBar)
{
    ...
    std::initializer_list<QWidget*> lblTitles = {ui->labelTitle1, ui->labelTitleAvailablezVpc, ui->labelTitle3, ui->labelTitle4, ui->labelTitlePendingzVpc, ui->labelTitleImmaturezVpc};
...
void TopBar::loadWalletModel()
{
    ...
    // update the display unit, to not use the default ("VoltPotCoin")
    updateDisplayUnit();
                            ...
                            const CAmount& delegatedBalance, const CAmount& coldStakedBalance)
{
    ...
    // VPC Total
    CAmount vpcAvailableBalance = balance;
    // zVPC Balance
    CAmount matureZerocoinBalance = zerocoinBalance - unconfirmedZerocoinBalance - immatureZerocoinBalance;

    // Set
    QString totalVpc = GUIUtil::formatBalance(vpcAvailableBalance, nDisplayUnit);
    QString totalzVpc = GUIUtil::formatBalance(matureZerocoinBalance, nDisplayUnit, true);

    // VPC
    // Top
    ui->labelAmountTopVpc->setText(totalVpc);
    // Expanded
    ui->labelAmountVpc->setText(totalVpc);
    ui->labelPendingVpc->setText(GUIUtil::formatBalance(unconfirmedBalance, nDisplayUnit));
    ui->labelImmatureVpc->setText(GUIUtil::formatBalance(immatureBalance, nDisplayUnit));

    // Update display state and/or values for zVPC balances as necessary
    bool fHaveZerocoins = zerocoinBalance > 0;

    // Set visibility of zVPC label titles/values
    ui->typeSpacerTop->setVisible(fHaveZerocoins);
    ui->typeSpacerExpanded->setVisible(fHaveZerocoins);
    ui->labelAmountTopzVpc->setVisible(fHaveZerocoins);
    ui->zerocoinBalances->setVisible(fHaveZerocoins);

    // Top
    ui->labelAmountTopzVpc->setText(totalzVpc);
    // Expanded
    ui->labelAvailablezVpc->setText(totalzVpc);
    ui->labelPendingzVpc->setText(GUIUtil::formatBalance(unconfirmedZerocoinBalance, nDisplayUnit, true));
    ui->labelImmaturezVpc->setText(GUIUtil::formatBalance(immatureZerocoinBalance, nDisplayUnit, true));
}
