...
#include "qt/voltpotcoin/forms/ui_addresseswidget.h"
#include "qt/voltpotcoin/addresslabelrow.h"
#include "qt/voltpotcoin/addnewaddressdialog.h"
...
AddressesWidget::AddressesWidget(VoltPotCoinGUI* parent) :
    PWidget(parent),
    ui(new Ui::AddressesWidget)
{
    ...
    // Address
    ui->labelAddress->setText(tr("Enter VoltPotCoin address"));
...
void AddressesWidget::onStoreContactClicked(){
    if (walletModel) {
        ...
        CBitcoinAddress vpcAdd = CBitcoinAddress(address.toUtf8().constData());
        ...
        if (walletModel->updateAddressBookLabels(vpcAdd.Get(), label.toUtf8().constData(),
                vpcAdd.IsStakingAddress() ? AddressBook::AddressBookPurpose::COLD_STAKING_SEND : AddressBook::AddressBookPurpose::SEND)
                ) {
