// Copyright (c) 2018-2020 The VoltPotCoin developers
// Distributed under the MIT software license, see the accompanying
// file COPYING or http://www.opensource.org/licenses/mit-license.php.

#include <zvpc/deterministicmint.h>
#include "zvpctracker.h"
#include "util.h"
#include "sync.h"
#include "main.h"
#include "txdb.h"
...
CzVPCTracker::CzVPCTracker(std::string strWalletFile)
{
...
CzVPCTracker::~CzVPCTracker()
{
    mapSerialHashes.clear();
    mapPendingSpends.clear();
}

void CzVPCTracker::Init()
{
...
bool CzVPCTracker::Archive(CMintMeta& meta)
{
...
bool CzVPCTracker::UnArchive(const uint256& hashPubcoin, bool isDeterministic)
{
    CWalletDB walletdb(strWalletFile);
    if (isDeterministic) {
        CDeterministicMint dMint;
...
CMintMeta CzVPCTracker::Get(const uint256 &hashSerial)
{
...
CMintMeta CzVPCTracker::GetMetaFromPubcoin(const uint256& hashPubcoin)
{
...
bool CzVPCTracker::GetMetaFromStakeHash(const uint256& hashStake, CMintMeta& meta) const
{
    for (auto& it : mapSerialHashes) {
...
std::vector<uint256> CzVPCTracker::GetSerialHashes()
{
...
CAmount CzVPCTracker::GetBalance(bool fConfirmedOnly, bool fUnconfirmedOnly) const
{
    CAmount nTotal = 0;
...
CAmount CzVPCTracker::GetUnconfirmedBalance() const
{
    return GetBalance(false, true);
}

std::vector<CMintMeta> CzVPCTracker::GetMints(bool fConfirmedOnly) const
{
...
bool CzVPCTracker::HasMintTx(const uint256& txid)
{
    for (auto it : mapSerialHashes) {
...
bool CzVPCTracker::HasPubcoin(const CBigNum &bnValue) const
{
...
bool CzVPCTracker::HasPubcoinHash(const uint256& hashPubcoin) const
{
    for (auto it : mapSerialHashes) {
        CMintMeta meta = it.second;
        if (meta.hashPubcoin == hashPubcoin)
...
bool CzVPCTracker::HasSerial(const CBigNum& bnSerial) const
{
    uint256 hash = GetSerialHash(bnSerial);
    return HasSerialHash(hash);
}

bool CzVPCTracker::HasSerialHash(const uint256& hashSerial) const
{
    auto it = mapSerialHashes.find(hashSerial);
    return it != mapSerialHashes.end();
}

bool CzVPCTracker::UpdateZerocoinMint(const CZerocoinMint& mint)
{
...
bool CzVPCTracker::UpdateState(const CMintMeta& meta)
{
    CWalletDB walletdb(strWalletFile);
...
void CzVPCTracker::Add(const CZerocoinMint& mint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzVPCTracker::SetPubcoinUsed(const uint256& hashPubcoin, const uint256& txid)
{
...
void CzVPCTracker::SetPubcoinNotUsed(const uint256& hashPubcoin)
{
    if (!HasPubcoinHash(hashPubcoin))
        return;
    CMintMeta meta = GetMetaFromPubcoin(hashPubcoin);
...
void CzVPCTracker::RemovePending(const uint256& txid)
{
    uint256 hashSerial;
...
bool CzVPCTracker::UpdateStatusInternal(const std::set<uint256>& setMempool, CMintMeta& mint)
{
...
std::set<CMintMeta> CzVPCTracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus, bool fWrongSeed, bool fExcludeV1)
{
    CWalletDB walletdb(strWalletFile);
    if (fUpdateStatus) {
        ...
        CzVPCWallet* zVPCWallet = new CzVPCWallet(strWalletFile);
        ...
        delete zVPCWallet;
        LogPrint("zero", "%s: added %d dzvpc from DB\n", __func__, listDeterministicDB.size());
    }
