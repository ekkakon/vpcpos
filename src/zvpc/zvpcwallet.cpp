...
CzVPCWallet::CzVPCWallet(std::string strWalletFile)
{
    ...
    //Check for old db version of storing zvpc seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZVPCSeed_deprecated(seed)) {
            ...
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZVPCSeed_deprecated()) {
                    ...
                    LogPrintf("%s: failed to remove old zvpc seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of zvpc wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool CzVPCWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    else if (!walletdb.ReadZVPCCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzVPCWallet::Lock()
{
    seedMaster = 0;
}

void CzVPCWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzVPCWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZVPC(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzVPCWallet::LoadMintPoolFromDB()
{
...
void CzVPCWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzVPCWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzVPCWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (pwalletMain->zvpcTracker->HasPubcoinHash(pMint.first)) {
...
bool CzVPCWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZVPC(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    // Add to zvpcTracker which also adds to database
    pwalletMain->zvpcTracker->Add(dMint, true);
...
void CzVPCWallet::SeedToZVPC(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzVPCWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzVPCWallet::GenerateDeterministicZVPC(CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzVPCWallet::GenerateMint(const uint32_t& nCount, const CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzVPCWallet::CheckSeed(const CDeterministicMint& dMint)
{
...
bool CzVPCWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
    if (!CheckSeed(dMint)) {
        uint256 hashSeed = Hash(seedMaster.begin(), seedMaster.end());
