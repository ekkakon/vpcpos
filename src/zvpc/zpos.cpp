...
bool CLegacyZVpcStake::InitFromTxIn(const CTxIn& txin)
{
    // Construct the stakeinput object
    if (!txin.IsZerocoinSpend())
        return error("%s: unable to initialize CLegacyZVpcStake from non zc-spend");
    ...
    *this = CLegacyZVpcStake(spend);

    // Find the pindex with the accumulator checksum
    if (!GetIndexFrom())
        return error("%s : Failed to find the block index for zvpc stake origin", __func__);

    // All good
    return true;
}

CLegacyZVpcStake::CLegacyZVpcStake(const libzerocoin::CoinSpend& spend)
{
...
CBlockIndex* CLegacyZVpcStake::GetIndexFrom()
{
...
CAmount CLegacyZVpcStake::GetValue() const
{
    return denom * COIN;
}

CDataStream CLegacyZVpcStake::GetUniqueness() const
{
...
bool CLegacyZVpcStake::ContextCheck(int nHeight, uint32_t nTime)
{
    const Consensus::Params& consensus = Params().GetConsensus();
    if (nHeight < consensus.height_start_ZC_SerialsV2 || nHeight >= consensus.height_last_ZC_AccumCheckpoint)
        return error("%s : zVPC stake block: height %d outside range", __func__, nHeight);
