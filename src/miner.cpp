...
CBlockTemplate* CreateNewBlock(const CScript& scriptPubKeyIn, CWallet* pwallet, bool fProofOfStake)
{
...

    {
        ...
        for (std::map<uint256, CTxMemPoolEntry>::iterator mi = mempool.mapTx.begin();
            ...
            for (const CTxIn& txin : tx.vin) {
                ...
                if (hasZerocoinSpends) {
                    ...
                    //Priority = (age^6+100000)*amount - gives higher priority to zvpcs that have been in mempool long
                    //and higher priority to zvpcs that are large in value
                    ...
                    // zVPC spends can have very large priority, use non-overflowing safe functions
        ...
        while (!vecPriority.empty()) {
            ...
            // double check that there are no double spent zVPC spends in this block or tx
            if (tx.HasZerocoinSpendInputs()) {
                ...
                //This zVPC serial has already been included in the block, do not add this tx.
                if (fDoubleSerial)
                    continue;
            }
        ...
        if (fProofOfStake) {
            ...
            if (pblock->IsZerocoinStake()) {
                ...
                if (!pwallet->GetZerocoinKey(bnSerial, key)) {
                    LogPrintf("%s: failed to find zVPC with serial %s, unable to sign block\n", __func__, bnSerial.GetHex());
                    return NULL;
                }
...
bool ProcessBlockFound(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    if (!ProcessNewBlock(state, NULL, pblock)) {
        if (pblock->IsZerocoinStake()) {
            pwalletMain->zvpcTracker->RemovePending(pblock->vtx[1].GetHash());
            pwalletMain->zvpcTracker->ListMints(true, true, true); //update the state
        }
        return error("VoltPotCoinMiner : ProcessNewBlock, block not accepted");
    }
...
{
    LogPrintf("VoltPotCoinMiner started\n");
    ...
    while (fGenerateBitcoins || fProofOfStake) {
        ...
        LogPrintf("Running VoltPotCoinMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void static ThreadBitcoinMiner(void* parg)
{
    ...
    try {
        ...
        LogPrintf("VoltPotCoinMiner exception");
    } catch (...) {
        LogPrintf("VoltPotCoinMiner exception");
    }

    LogPrintf("VoltPotCoinMiner exiting\n");
}
