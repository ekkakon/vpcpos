...
void PrepareShutdown()
{
    ...
    util::ThreadRename("voltpotcoin-shutoff");
...
{
    util::ThreadRename("voltpotcoin-loadblk");
...
bool AppInit2()
{
    ...
    // Sanity check
    if (!InitSanityCheck())
        return UIError(_("Initialization sanity check failed. VoltPotCoin Core is shutting down."));
    ...
    if (!lock.timed_lock(boost::get_system_time() + boost::posix_time::seconds(10)))
        return UIError(strprintf(_("Cannot obtain a lock on data directory %s. VoltPotCoin Core is probably already running."), strDataDir));
        ...
        do {
            ...
            try {
                ...
                if (fReindexMoneySupply) {
                    LOCK(cs_main);
                    // Skip zvpc if already reindexed
                    RecalculateVPCSupply(1, fReindexZerocoin);
                }
...
// ********************************************************* Step 8: load wallet
    ...
    if (fDisableWallet) {
        ...
        if (GetBoolArg("-upgradewallet", fFirstRun && !fLegacyWallet)) {
            if (prev_version <= FEATURE_PRE_VoltPotCoin && pwalletMain->IsLocked()) {
        ...
        if (fFirstRun) {
            if (!fLegacyWallet) {
                ...
                pwalletMain->SetMaxVersion(FEATURE_PRE_VoltPotCoin);
            }
